...
Handles both quick mode (10k samples) and full mode (100k samples) with proper stratified sampling.
"""
...
class DataValidator:
    ...
    def load_dataset(self) -> Dict[str, Any]:
        ...
        Full mode: 100k samples (80k train, 20k test)
        ...
        try:
            ...
            # Check if we're in quick mode and if sampled data already exists
            sampled_data_path = self.dataset_path / f"sampled_{self.test_mode}_{self.seed}"
            if sampled_data_path.exists():
                logger.info(f"Found existing sampled data for {self.test_mode} mode at {sampled_data_path}")
                return self._load_sampled_dataset(sampled_data_path)
    ...
    def _perform_stratified_sampling(self, train_labels, train_text, train_metadata,
        ...
        for i in range(sampled_train_text.shape[1]):
            train_data[f'text_feature_{i}'] = sampled_train_text[:, i]
        for i in range(sampled_train_metadata.shape[1]):
            train_data[f'metadata_feature_{i}'] = sampled_train_metadata[:, i]
        
        # Convert labels from 1-indexed to 0-indexed
        train_data['label'] = sampled_train_labels.astype(int) - 1
        dataset['train'] = pd.DataFrame(train_data)
        
        # Create test DataFrame
        test_data = {}
        for i in range(sampled_test_text.shape[1]):
            test_data[f'text_feature_{i}'] = sampled_test_text[:, i]
        for i in range(sampled_test_metadata.shape[1]):
            test_data[f'metadata_feature_{i}'] = sampled_test_metadata[:, i]
        
        # Convert labels from 1-indexed to 0-indexed
        test_data['label'] = sampled_test_labels.astype(int) - 1
        dataset['test'] = pd.DataFrame(test_data)
        
        # Verify label conversion
        train_labels_final = dataset['train']['label'].unique()
        test_labels_final = dataset['test']['label'].unique()
        ...
        final_train_unique, final_train_counts = np.unique(dataset['train']['label'], return_counts=True)
        final_test_unique, final_test_counts = np.unique(dataset['test']['label'], return_counts=True)
    ...
    def _apply_sampling(self, dataset: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        for split_name, df in dataset.items():
            ...
            if sample_size < original_size:
                sampled_df = df.sample(n=sample_size, random_state=self.seed)
                logger.info(f"Sampled {split_name}: {original_size} -> {sample_size} samples")
            else:
                sampled_df = df.copy()
...
def run_phase_1_data_validation(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        results.update({
            ...
            "processed_data": {
                "train": dataset["train"].to_csv(index=False) if "train" in dataset else None,
                "test": dataset["test"].to_csv(index=False) if "test" in dataset else None,
                "task_type": "classification"  # Amazon Reviews is classification
            },
            "label_analysis": {
                "train_labels": dataset["train"]["label"].unique().tolist() if "train" in dataset and "label" in dataset["train"].columns else None,
                "test_labels": dataset["test"]["label"].unique().tolist() if "test" in dataset and "label" in dataset["test"].columns else None,
                "label_range": {
                    "min": int(dataset["train"]["label"].min()) if "train" in dataset and "label" in dataset["train"].columns else None,
                    "max": int(dataset["train"]["label"].max()) if "train" in dataset and "label" in dataset["train"].columns else None
                }
            }
        })
...
if __name__ == "__main__":
    ...
    test_config = {
        ...
        "seed": 42,
        "test_mode": "quick",  # Will sample 10k samples (8k train, 2k test)
