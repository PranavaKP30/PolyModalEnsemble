#!/usr/bin/env python3
"""
Configuration module for Amazon Reviews experiments
"""
...
class ExperimentConfig:
    """Configuration for Amazon Reviews experiments"""
    ...
    task_type: str = "5-class rating prediction"
    n_classes: int = 5
...
class PathConfig:
    ...
    def __post_init__(self):
        ...
        if self.data_path is None:
            self.data_path = self.project_root / "Benchmarking" / "PreprocessedData" / "AmazonReviews"
        
        if self.output_path is None:
            self.output_path = self.project_root / "Benchmarking" / "Experiments" / "AmazonReviews" / "results"
        
        if self.metrics_path is None:
            self.metrics_path = self.project_root / "Benchmarking" / "Experiments" / "AmazonReviews" / "metrics"
...
def print_config_summary(exp_config: ExperimentConfig, path_config: PathConfig):
    ...
    print(f"🛒 Amazon Reviews Experiment Configuration")
    print("=" * 60)
    ...
    print(f"🎯 Task: {exp_config.n_classes}-class rating prediction")
