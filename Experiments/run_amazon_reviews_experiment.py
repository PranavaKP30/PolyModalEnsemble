...
class AmazonReviewsExperiment:
    ...
    def _get_experiment_config(self) -> Dict[str, Any]:
        ...
        if self.test_mode == "quick":
            config = {
                ...
                # Core ensemble parameters (Most Important) - QUICK MODE
                "n_bags": [3, 5],  # Number of ensemble bags - REDUCED for quick testing
                ...
                # Training parameters (High Impact) - QUICK MODE
                "epochs": [5, 10],  # Training epochs - REDUCED for quick testing
                "batch_size": [32, 64],  # Batch size
                "early_stopping_patience": [3, 5],  # Early stopping patience - REDUCED
            ...
            config = {
                # Dataset configuration - Phase 1 will handle sampling
                "test_mode": "full",  # Phase 1 will use 300k samples (240k train, 60k test)
    ...
    def run_phase(self, phase_name: str, seed: int, processed_data: Dict[str, Any] = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        # Use full dataset but with reduced parameters for quick mode
        phase_config = {
            **self.config,
            "seed": seed,
            ...
            for phase in phases:
                ...
                if phase_success:
                    ...
                    if phase == "phase_1_data_validation" and phase_results.get("status") == "completed":
                        if phase_results.get("processed_data") is not None:
                            processed_data = phase_results.get("processed_data")
                            logger.info(f"Extracted processed data from {phase} for subsequent phases")
...
def main():
    ...
    if test_mode is None:
        ...
        print("   • Single seed")
        print("   • Estimated time: 5-10 minutes (Phase 1 + 2)")
        print()
        print("2. Full Test - Complete evaluation")
        print("   • Uses 100,000 samples (80,000 train, 20,000 test)")
        print("   • Stratified sampling maintains original distribution")
        print("   • Multiple seeds")
        print("   • Estimated time: 20-30 minutes (Phase 1 + 2)")
        print()
        print("Note: Currently Phase 1 (Data Validation) and Phase 2 (Baseline Models) are implemented.")
        print("Phase 3 (MainModel HPO) and beyond will be added in future updates.")
        print()
